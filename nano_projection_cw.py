...
for ct, p in enumerate(psr):
    ...
    C += PALutils.createRedNoiseCovarianceMatrix(tm.copy(), A_red, gam_red)
...
fname = 'upper_{0}_{1}_{2}'.format(freq, opts.nreal, npsr)
#if opts.pulsar is not None:
#    fname += '_{0}'.format(opts.pulsar)
fname += '.txt'
...
# save arrays
#snrName=outDir+'/snr_data_{0}_{1}_{2}_{3}_{4}.npy'.format(opts.npsr,A_gw,A_red,gam_red,opts.label)
#snrName2=outDir+'/snr_{0}_{1}_{2}_{3}_{4}.npy'.format(opts.npsr,A_gw,A_red,gam_red,opts.label)
#np.save(snrName,rho)
#a=np.zeros((Nt,3))
#a[:,0]=t
#a[:,1]=meanSnr
